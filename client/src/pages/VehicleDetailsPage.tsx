...
export default function VehicleDetailsPage() {
  ...
  const details = trpc.vehicles.details.useQuery(
    { id },
    { enabled: Number.isInteger(id) && id > 0 }
  );
  const row = details.data;
  const currentContract = row?.contracts.find(({ contract }) =>
    ["active", "overdue"].includes(contract.status)
  );
  const oilMaintenance = row
    ? calculateOilMaintenance({
        currentMileage: row.vehicle.mileage,
        lastOilChangeMileage: row.vehicle.lastOilChangeMileage,
        oilChangeInterval: row.vehicle.oilChangeInterval,
      })
    : null;
  const canMaintainVehicle = canPerform(
    user?.role ?? "user",
    user?.permissions,
    "vehicles.maintenance"
  );
  const canDeleteVehicle = canPerform(
    user?.role ?? "user",
    user?.permissions,
    "vehicles.delete"
  );
      ...
      <div dir="rtl" className="mx-auto max-w-5xl space-y-5">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="icon"
            aria-label="العودة إلى السيارات"
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              بيانات السيارة والعداد والوثائق والعقود والعمليات المرتبطة بها.
            </p>
          </div>
        </div>
        ...
        {details.error && (
          <Card>
            <CardContent className="py-12 text-center text-sm text-red-600">
              تعذر تحميل السيارة: {details.error.message}
            </CardContent>
          </Card>
        )}
          ...
          <>
              ...
              <CardContent className="flex flex-wrap items-center justify-between gap-4 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-300">
                      لوحة {row.vehicle.plateNumber} · موديل{" "}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <Badge className="bg-white/10 text-white hover:bg-white/10">
                    {statusLabels[row.vehicle.status] ?? row.vehicle.status}
                  </Badge>
                </div>
              </CardContent>
            </Card>
            ...
            {canDeleteVehicle && (
              <VehicleDeleteAction
                ...
                onDeleted={() => setLocation("/vehicles")}
              />
            )}
            ...
            {currentContract && (
              <Card className="border-0 bg-[#effaf8] shadow-sm">
                <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
                  <div>
                    <p className="text-xs font-bold text-[#0c8f87]">
                      العقد الحالي والعميل الحالي
                    </p>
                    <p className="mt-1 text-sm font-black text-slate-700">
                      {currentContract.customer?.fullName ?? "عميل غير متاح"} ·
                      العقد #{currentContract.contract.contractNumber}
                    </p>
                  </div>
                  <Link
                    href={`/contracts/${currentContract.contract.id}`}
                    className="rounded-lg bg-white px-3 py-2 text-xs font-bold text-[#0c8f87] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16b4a5]"
                  >
                    فتح العقد
                  </Link>
                </CardContent>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Info
                icon={Gauge}
                label="العداد الحالي"
                value={`${row.vehicle.mileage.toLocaleString()} كم`}
              />
              <Info
                icon={Wrench}
                label="عداد آخر تغيير زيت"
                value={
                  row.vehicle.lastOilChangeMileage != null
                    ? `${row.vehicle.lastOilChangeMileage.toLocaleString()} كم`
                    : "غير مسجل"
                }
              />
              <Info
                ...
                value={date(row.vehicle.lastOilChangeDate)}
              />
              <Info
                icon={Wrench}
                label="المتبقي للصيانة القادمة"
                value={
                  oilMaintenance?.remaining == null
                    ? "غير محدد"
                    : oilMaintenance.due
                      ? "مستحق الآن"
                      : `${oilMaintenance.remaining.toLocaleString()} كم`
                }
              />
              <Info
                icon={ClipboardList}
                label="عدد العقود"
                value={String(row.contracts.length)}
              />
              <Info
                icon={FileCheck}
                label="انتهاء التأمين"
                value={date(row.vehicle.insuranceExpiryDate)}
              />
              <Info
                icon={FileCheck}
                label="انتهاء الفحص الدوري"
                value={date(row.vehicle.inspectionExpiryDate)}
              />
              <Info
                icon={FileCheck}
                label="انتهاء الاستمارة"
                value={date(row.vehicle.registrationExpiryDate)}
              />
            </div>
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle>
                  العقود التاريخية للسيارة ({row.contracts.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {row.contracts.length ? (
                  <div className="space-y-2">
                    {row.contracts.map(({ contract, customer }) => (
                      <Link
                        ...
                        className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-100 px-4 py-3 text-sm transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16b4a5]"
                      >
                        <div>
                          <p className="font-bold text-[#0c8f87]">
                            العقد #{contract.contractNumber}
                          </p>
                          <p className="mt-1 text-xs text-slate-500">
                            {customer?.fullName ?? "عميل غير متاح"} · من{" "}
                            {date(contract.startDate)} إلى{" "}
                            {date(contract.expectedReturnDate)}
                          </p>
                        </div>
                  ...
                  <p className="py-6 text-sm text-slate-400">
                    لا توجد عقود تاريخية لهذه السيارة.
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle>
                  سجل الصيانة وتغيير الزيت ({row.maintenance.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {row.maintenance.length ? (
                  <div className="space-y-2">
                    {row.maintenance.map(item => (
                      <div
                        key={item.id}
                      ...
                      >
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-400">
                            التاريخ: {date(item.startDate)} · العداد:{" "}
                            {item.mileage != null
                              ? `${item.mileage.toLocaleString()} كم`
                              : "غير مسجل"}
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle>
                  عمليات السيارة والملاحظات ({row.operations.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {row.operations.length ? (
                  ...
                  <p className="py-6 text-sm text-slate-400">
                    لا توجد عمليات مرتبطة بهذه السيارة.
                  </p>
                )}
              </CardContent>
            </Card>
...
function VehicleDeleteAction({
  ...
  return (
    <Card className="border border-red-100 bg-red-50/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-base text-red-800">حذف السيارة</CardTitle>
        <p className="text-xs text-red-700">
          لا يسمح بالحذف إذا كانت السيارة مرتبطة بعقد أو سجل صيانة، حفاظاً على
          التاريخ المالي والتشغيلي.
        </p>
      </CardHeader>
      <CardContent>
        <Button
          ...
          onClick={() => {
            if (
              window.confirm(
                `هل تريد حذف السيارة ${plateNumber} نهائياً؟ لا يمكن التراجع عن هذه العملية.`
              )
            )
              remove.mutate({ id: vehicleId });
          }}
        >
          <Trash2 className="h-4 w-4" />
          {remove.isPending ? "جارٍ الحذف..." : "حذف السيارة"}
        </Button>
      </CardContent>
    </Card>
  );
...
function VehicleEditor({
  ...
  const fields = [
    ...
    ["insuranceExpiryDate", "تاريخ تجديد التأمين", "date"],
    ["inspectionExpiryDate", "تاريخ تجديد الفحص الدوري", "date"],
    ["registrationExpiryDate", "تاريخ تجديد الاستمارة", "date"],
  ] as const;
    ...
    <Card className="border-0 shadow-sm">
      <CardHeader>
        <CardTitle className="text-base">
          تحديث بيانات السيارة والوثائق
        </CardTitle>
        <p className="text-xs leading-6 text-slate-400">
          حدّث العداد من هنا عند الحاجة، وسجّل تاريخ التجديد الجديد للتأمين أو
          الفحص الدوري أو الاستمارة ليظهر في ملف السيارة والتنبيهات.
        </p>
      </CardHeader>
      <CardContent>
        ...
        <Button
          onClick={() =>
            update.mutate({
              ...
              lastOilChangeMileage: values.lastOilChangeMileage
                ? Number(values.lastOilChangeMileage)
                : undefined,
              lastOilChangeDate: values.lastOilChangeDate,
              oilChangeInterval: Number(values.oilChangeInterval || 5000),
          ...
          className="mt-4 min-h-11 bg-[#139f95] hover:bg-[#0d938b]"
        >
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardContent className="p-4">
        <Icon className="h-5 w-5 text-[#139f95]" />
