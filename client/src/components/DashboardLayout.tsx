...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const changePassword = trpc.auth.changePassword.useMutation({
    onSuccess: () => { setCurrentPassword(""); setNewPassword(""); setPasswordConfirmation(""); toast.success("تم تغيير كلمة المرور"); },
    onError: (error) => toast.error(error.message),
  });
  const submitPasswordChange = (event: React.FormEvent) => {
    event.preventDefault();
    if (newPassword.length < 8) return toast.error("كلمة المرور الجديدة يجب أن تتكون من 8 أحرف على الأقل");
    ...
    return (
      <div dir="rtl" className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#eef4f7] p-4 sm:p-6">
        ...
        <div className="relative grid w-full max-w-5xl overflow-hidden rounded-[28px] border border-white/80 bg-white shadow-[0_24px_70px_rgba(11,39,71,0.16)] lg:grid-cols-[1.05fr_0.95fr]">
          <section aria-label="نبذة عن مكتب مشاري لتأجير السيارات" className="hidden min-h-[560px] flex-col justify-between bg-[#0b2747] p-10 text-white lg:flex">
            <div><div className="flex items-center gap-2 text-2xl font-black"><CarFront className="h-7 w-7 text-[#16b4a5]" /><span>مكتب مشاري لتأجير السيارات</span></div><p className="mt-2 max-w-xs text-sm leading-7 text-slate-300">منصة تشغيل عربية تساعدك على متابعة العقود والسيارات والمدفوعات من مكان واحد.</p></div>
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#f5f7fa] text-[#172235]">
      ...
      <aside aria-label="القائمة الرئيسية" className={cn("fixed inset-y-0 right-0 z-50 flex w-[272px] flex-col bg-[#0b2747] text-white shadow-2xl transition-transform duration-200 lg:translate-x-0", open ? "translate-x-0" : "translate-x-full lg:translate-x-0")}>
        <div className="flex h-[92px] items-center justify-between border-b border-white/10 px-7"><div><div className="flex items-center gap-2 text-2xl font-black"><CarFront className="h-7 w-7 text-[#16b4a5]" /><span>مكتب مشاري لتأجير السيارات</span></div><p className="mt-1 text-[11px] text-slate-300">نظام إدارة العقود والحسابات</p></div><button type="button" className={`flex min-h-11 min-w-11 items-center justify-center rounded-lg p-2 text-slate-300 hover:bg-white/10 ${focusClass}`} onClick={() => setOpen(false)} aria-label="إغلاق القائمة"><X className="h-5 w-5" /></button></div>
