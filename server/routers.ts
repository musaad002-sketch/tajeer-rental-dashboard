...
export const appRouter = router({
  ...
  contracts: router({
    create: permissionProcedure("contracts.create").input(z.object({ contractNumber: z.string().trim().min(2).optional(), customerId: z.number().int().positive(), vehicleId: z.number().int().positive(), vehicleMileage: z.number().int().min(0), oilOverrideAcknowledged: z.boolean().optional(), type: z.enum(["daily", "monthly"]), contractScope: z.enum(["domestic_limited", "domestic_open", "international"]).optional(), startDate: z.string(), expectedReturnDate: z.string(), rentalAmount: z.string().min(1), days: z.number().int().positive(), totalAmount: z.string().min(1), paidAmount: z.string().optional(), initialCashAmount: z.string().optional(), initialNetworkAmount: z.string().optional(), notes: z.string().optional() })).mutation(({ input, ctx }) => createContract({ ...input, createdBy: ctx.user.id })),
