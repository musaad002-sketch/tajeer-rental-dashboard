...
export const appRouter = router({
  ...
  users: router({
    ...
    update: adminProcedure.input(z.object({ id: z.number().int().positive(), username: z.string().trim().min(3).max(64).optional(), name: z.string().trim().min(2).optional(), email: z.string().email().optional(), password: z.string().min(6).optional(), role: z.enum(["user", "admin"]).optional(), isActive: z.boolean().optional(), permissions: z.array(z.enum(permissionKeys)).optional() })).mutation(({ input, ctx }) => { if (input.id === ctx.user.id && input.isActive === false) throw new TRPCError({ code: "BAD_REQUEST", message: "لا يمكن تعطيل حسابك الحالي" }); return updateManagedUser(input); }),
    delete: adminProcedure.input(z.object({ id: z.number().int().positive() })).mutation(({ input, ctx }) => { if (input.id === ctx.user.id) throw new TRPCError({ code: "BAD_REQUEST", message: "لا يمكن حذف الحساب المستخدم حالياً" }); return deleteManagedUser(input.id); }),
  ...
  maintenance: router({
    ...
    updateStatus: adminProcedure.input(z.object({ id: z.number().int().positive(), vehicleId: z.number().int().positive(), status: z.enum(["pending", "in_progress", "completed", "written_off"]) })).mutation(({ input }) => updateMaintenanceStatus(input.id, input.status, input.vehicleId)),
